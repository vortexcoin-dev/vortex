...
// vortex: optional setting to unlock wallet for block staking only;
...
//    return n ? n : nTimeReceived;
    ...
    // vortex: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// vortex: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // vortex: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // vortex: set time
...
//                }
                ...
                // vortex: sub-cent change is moved to fee
...
// vortex: create coin stake transaction
