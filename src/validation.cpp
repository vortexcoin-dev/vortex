...
#if defined(NDEBUG)
# error "Vortex cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Vortex Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // vortex: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
// vortex: it depends on current POW Block Height, not current blockchain Height
...
// vortex: miner's coin stake is rewarded based on coin age spent (coin-days)
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // vortex: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // vortex
            undo.nTime = alternate.nTime;           // vortex
        } else {
...
bool VortexContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // vortex: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!VortexContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // vortex: coinbase reward check relocated to CheckBlock()
    ...
    // vortex: track money supply and mint amount info
    ...
    // vortex: fees are not collected by miners as in bitcoin
    // vortex: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // vortex: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // vortex: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // vortex: check transaction timestamp
...
#ifdef ENABLE_CHECKPOINTS
    // vortex: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // vortex: Don't reject in case of old clients. Change our assumption instead.
...
//    // vortex: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // vortex: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // vortex: check PoS
    if (fCheckPoS && !VortexContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // vortex: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // vortex: if responsible for sync-checkpoint send it
...
#ifdef ENABLE_CHECKPOINTS
    // vortex: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // vortex: txindex is always enabled
...
// vortex: total coin age spent in transaction, in the unit of coin-days.
