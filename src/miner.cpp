...
// vortex: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr <CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, bool fMineWitnessTx, CWallet *pwallet, bool *pfPoSCancel) {
    ...
    // vortex: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // vortex: there is no point to continue if we failed to create coinstake
    }
...
//        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VortexMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet) {
    ...
    RenameThread("vortex-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            // vortex: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake()) {
...
void MintStake(boost::thread_group &threadGroup) {
    // vortex: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
...
void static VortexMiner(const CChainParams &chainparams, void *parg) {
    LogPrintf("VortexMiner started\n");
    RenameThread("vortex-miner");
    CWallet *pwallet = (CWallet *) parg;
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in VortexMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running VortexMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        LOCK2(cs_main, pwallet->cs_wallet);
                        if (!SignBlock(*pblock, *pwallet)) {
                            LogPrintf("VortexMiner(): failed to sign PoS block");
                            continue;
                        }
        ...
        LogPrintf("VortexMiner terminated!\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("VortexMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateVortex(bool fGenerate, int nThreads, const CChainParams &chainparams) {
    ...
    if (!vpwallets.empty()) {
        for (int i = 0; i < nThreads; i++)
            minerThreads->create_thread(boost::bind(&VortexMiner, boost::cref(chainparams), vpwallets[0]));
    }
}
